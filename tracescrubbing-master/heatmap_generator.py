...
class HeatmapGenerator(object):
    ...
    def getHeatmapMatrix(self, trace, hm_height, window_size):
        ...
        #cols = (np.ceil(1. * 16078 / window_size)).astype(int)
        ...
        #for idx in range(len(trace)):
            ...
            if trace[i] > max_addr:
                max_addr = trace[i]
            matrix[trace[i] % rows][idx // window_size] += 1
            i = i + 1
    ...
    def compareHeatmaps(self, heatmap_orig, heatmap_new, name, save=False):
        ...
        f, ax = plt.subplots(1, 2, figsize=(14,6), sharex=True, frameon=True)
        ax[0].imshow(heatmap_orig,cmap=cm.gray_r)
        ax[0].set_title('Heatmap of source trace: {}'.format(name))
        ax[0].axis('off')
        ax[1].imshow(heatmap_new,cmap=cm.gray_r)
